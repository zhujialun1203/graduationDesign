  ...
  export default{
    ...
    methods: {
      ...
      loadBlogs(page, count){ 
        ...
        var url = '';
        if (this.state == -2) {
          url = "/api-article/admin/article/all" + "?page=" + page + "&count=" + count + "&keywords=" + this.keywords;
        } else {
          url = "/api-article/article/all?state=" + this.state + "&page=" + page + "&count=" + count + "&keywords=" + this.keywords;
        }
      ...
      handleRestore(index, row) {
        ...
        this.$confirm('将该文件还原到原处，是否继续？','提示',{
          ...
          _this.loading = true;
          putRequest('/api-article/article/restore', {articleId: row.id}).then(resp=> {
            if (resp.status == 200) {
              var data = resp.data;
