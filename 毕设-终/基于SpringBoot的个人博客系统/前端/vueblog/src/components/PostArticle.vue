  ...
  export default {
    mounted: function () {
      ...
      if (from != null && from != '' && from != undefined) {
        ...
        this.id = id;
        this.loading = true;
        getRequest("/api-article/article/" + id).then(resp=> {
          _this.loading = false;
          if (resp.status == 200) {
    ...
    methods: {
      ...
      saveBlog(state){
        ...
        var _this = this;
        _this.loading = true;
        postRequest("/api-article/article/", {
          id: _this.article.id,
...
//            }
      ...
      imgAdd(pos, $file){
        ...
        uploadFileRequest("/api-article/article/uploadimg", formdata).then(resp=> {
          var json = resp.data;
          if (json.status == 'success') {
//            _this.$refs.md.$imgUpdateByUrl(pos, json.msg)z
