  ...
  <div v-loading="loading">
    ...
    <div style="display: flex;justify-content: space-around;flex-wrap: wrap">
      <el-card style="width:330px;margin-top: 10px;" v-for="(user,index) in users" :key="index"
        ...
        <div>
          <div><img :src="user.avatar" :alt="user.nickname" style="width: 70px;height: 70px"></div>
          <div style="text-align: left;color:#20a0ff;font-size: 12px;margin-top: 13px">
            <span>用户名:</span><span>{{user.userName}}</span>
          </div>
          <div style="text-align: left;color:#20a0ff;font-size: 12px;margin-top: 13px">
            <span>用户昵称:</span><span>{{user.nickName}}</span>
          </div>
          <div style="text-align: left;color:#20a0ff;font-size: 12px;margin-top: 13px">
          ...
          <div style="text-align: left;color:#20a0ff;font-size: 12px;margin-top: 13px">
            ...
            <!-- <el-button @click="test(user.regTime)"></el-button> -->
          </div>
        </div>
      </el-card>
    </div>
...
//       },
      saveRoles(id, index){
        ...
        putRequest("/api-user/admin/user/role", {rids: this.roles, id: id}).then(resp=> {
      ...
      deleteUser(id){
        ...
        this.$confirm('删除该用户, 是否继续?', '提示', {
          ...
          _this.loading = true;
          deleteRequest("/api-user/admin/user/" + id).then(resp=> {
            if (resp.status == 200 && resp.data.status == 'success') {
      ...
      enabledChange(enabled, id, index){
        var _this = this;
        _this.cardloading.splice(index, 1, true)
        putRequest("/api-user/admin/user/enabled", {enabled: enabled, uid: id}).then(resp=> {
          if (resp.status != 200) {
            _this.$message({type: 'error', message: '更新失败!'})
            _this.loadOneUserById(id, index);
            return;
      ...
      loadRoles(index){
        ...
        getRequest("/api-user/admin/roles").then(resp=> {
      ...
      loadOneUserById(id, index){
        var _this = this;
        getRequest("/api-user/admin/user/" + id).then(resp=> {
          _this.cardloading.splice(index, 1, false)
          if (resp.status == 200) {
      ...
      loadUserList(){
        var _this = this;
        getRequest("/api-user/admin/user?nickname="+this.keywords).then(resp=> {
          _this.loading = false;
