  ...
  export default{
    methods: {
      addNewCate(){
        this.loading = true;
        var _this = this;
        postRequest('/api-article/admin/category/', {cateName: this.cateName}).then(resp=> {
          if (resp.status == 200) {
            var json = resp.data;
      ...
      handleEdit(index, row){
        ...
        this.$prompt('请输入新名称', '编辑', {
          ...
          if (value == null || value.length == 0) {
            ...
            _this.loading = true;
            putRequest("/api-article/admin/category/", {id: row.id, cateName: value}).then(resp=> {
              var json = resp.data;
              _this.$message({
                type: json.status,
                message: json.msg
      ...
      deleteCate(ids){
        ...
        this.loading = true;
        //删除
        deleteRequest("/api-article/admin/category/" + ids).then(resp=> {
          var json = resp.data;
          _this.$message({
            type: json.status,
            message: json.msg
      ...
      refresh(){
        let _this = this;
        getRequest("/api-article/category/all").then(resp=> {
